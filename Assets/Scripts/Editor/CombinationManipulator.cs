...
namespace MadPot
{
    public class CombinationManipulator : MouseManipulator
    {
        private Combination _combination;
        private CombinationDraw _levelDraw;

        public CombinationManipulator(Combination combination)
        {
            activators.Add(new ManipulatorActivationFilter { button = MouseButton.LeftMouse });
            _combination = combination;
            _levelDraw = new CombinationDraw { products = combination.Products };
        }
        ...
        {
            var t = target as CombinationEditorGraphView;
        ...
        private void MouseUp(MouseUpEvent evt)
        {
            ...
            if (_combination.Products.Count == 0) _combination.Products.Add(new LevelProduct() { Position = _levelDraw.start });

            var t = target as CombinationEditorGraphView;
            _combination.Products.Add(new LevelProduct() { Position = t.ScreenToWorldSpace(evt.localMousePosition) });
        ...
        private void MouseDown(MouseDownEvent evt)
        {
            ...
            if (_combination.Products.Count != 0) _levelDraw.start = _combination.Products.Last().Position;
            else _levelDraw.start = t.ScreenToWorldSpace(evt.localMousePosition);

            _levelDraw.end = t.ScreenToWorldSpace(evt.localMousePosition);
            _levelDraw.MarkDirtyRepaint();
        }
        private class CombinationDraw : ImmediateModeElement
        {
            ...
            protected override void ImmediateRepaint()
            {
                ...
                var t = parent as CombinationEditorGraphView;
